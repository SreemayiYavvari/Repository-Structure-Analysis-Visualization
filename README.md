...
Here is the updated paragraph tailored for Gemini:

The default Gemini model is **`gemini-2.5-flash`** — fast and inexpensive, 
which suits short per‑file summaries. Providers are implemented over plain HTTP (`httpx`), so no vendor SDK is required..
